...
# this is used to match relations to classes by predicting if an association relation ('.') touches another relation.
    def calculateLine(self, coordAX, coordAY, coordBX, coordBY):
    ...
    def parseUMLetino(self, filename):
    ...
    def matchRelationToClasses(self, classes, relations):
                ...
                for rel2 in relations:
                    if rel2.Linetype != '.':
                        ...
                        for i in range(len(relAdd) - 1):
                            ...
                            m, c = self.calculateLine(int(relAdd2[i].split(",")[0]) + rel2.X,
    ...
    def repeatNames(self):
        print("The following classes Repeat their names")
        for C in self.classes:
    ...
    def classPlurals(self):
        inf = inflect.engine()
        countSingle = 0
        countPlural = 0
        for C in self.classes:
            if not inf.singular_noun(C.Title):
                print(C.Title, "is singular")
                countSingle = countSingle + 1
            else:
                print(C.Title, "is plural")
                countPlural = countPlural + 1
        print("You have used", countSingle, "singular names and", countPlural, "Plural names")
